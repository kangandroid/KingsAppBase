...
mixin 重用代码的方法。mixin 不能有构造函数，可以用class和mixin关键字去定义，使用关键字 mixin 替代 class 让其成为一个单纯的 Mixin 类。使用关键字 mixin 时可以用on关键字来指定混入类型
...
Dart在单线程中是以消息循环机制来运行的，其中包含两个任务队列，一个是“微任务队列” microtask queue，
另一个叫做“事件队列” event queue。微任务队列的执行优先级高于事件队列。
...
Widget 是用来描述Element的对象的配置信息，通过createElement 创建出Element对象。
widget是不可变的。State描述状态的，State的改变会build出新的widget。
从而产生新的Element。 Element树中保存了页面逻辑，
...
1. Platform Thread flutterEngine 运行的线程。（）
2. UI Thread [DartExecuter]用来启动DartVM 运行Flutter 所在Isolate。（）
3. GPU Thread 负责接应UIThread 将Flutter像素绘制到Android{@code View}层次结构中。[FlutterRenderer 有JNI调用]
4. IO Thread  进行IO操作，图片资源的存取，磁盘文件的读写。
...
在Key属性控制一个Widget如何取代另一个控件树。如果两个窗口Widget的runtimeType和key属性分别是相同时，则新窗口Widget通过更新底层的Element（即，通过使用新窗口Widget调用Element.update）来替换旧窗口 Widget。否则，将从树中删除旧Element，将新Widget infalte成为一个Element，然后将新Element插入到树中。

1. StatefulWidget和State，用于可以在其生命周期内多次构建的Widget。
2. InheritedWidget，用于引入可被后代Widget读取的环境状态的Widget。
. StatelessWidget，用于在特定配置和环境状态下始终以相同方式构建的Widget。
...
* 1.控制 build() 方法的耗时。‘要确保在8ms内完成’ 避免在 build() 方法中进行重复且耗时的工作，因为当父 widget 重建时，子 Wdiget 的 build() 方法会被频繁地调用。
* 2.仅当需要的时候才应用效果
* 3.对列表和网格列表懒加载
* 4.多用常量Widget。
...
###ChangeNotifier 
实现了 Listenable 接口 可以作为接口和mixin来使用
子类 ScrollController 
ValueNotifier 持有数据可变数据 value实时向 Listener通知数据变化
