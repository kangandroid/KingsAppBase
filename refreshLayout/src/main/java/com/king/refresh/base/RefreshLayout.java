...
import com.king.refresh.normalstyle.NestHeader;
...
public abstract class RefreshLayout extends ViewGroup implements NestedScrollingChild, NestedScrollingParent {
    ...
    //region field
    protected static final String TAG = RefreshLayout.class.getSimpleName();
    ...
    public RefreshLayout(View listView) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (attr == R.styleable.RefreshLayout_footerNestLayout) {
    ...
    public void setLoadingView(View view) {
        if (mLodingView == view)
            return;
        if (mLodingView != null)
            removeViewInLayout(mLodingView);
        mLodingView = view;
        if (mLodingView.getParent() != null) {
            ((ViewGroup) mLodingView.getParent()).removeView(mLodingView);
        }
        mLodingView.setVisibility(GONE);
        if (mTargetView != null)
            addView(mLodingView, 1, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        else
            addView(mLodingView, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
    }
     ...
     * @param view Empty View
     */
    public void setmNetErrorViewView(View view) {
        if (mNetErrorView == view)
            return;
        if (mNetErrorView != null)
            removeViewInLayout(mNetErrorView);
        mNetErrorView = view;
        if (mNetErrorView.getParent() != null) {
            ((ViewGroup) mNetErrorView.getParent()).removeView(mNetErrorView);
        }
        mNetErrorView.setVisibility(GONE);
        if (mTargetView != null)
            addView(mNetErrorView, 1, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
        else
    ...
    /**
     * @see android.widget.LinearLayout (int, int)
     */
    @Override
